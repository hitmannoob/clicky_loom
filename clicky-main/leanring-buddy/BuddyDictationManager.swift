...
final class BuddyDictationManager: NSObject, ObservableObject {
    ...
    private func startPushToTalk(
        ...
        LogGuru.debug(
            "BuddyDictationManager start requested (\(startSource))",
            category: .transcription
        )

        if needsInitialPermissionPrompt {
            LogGuru.info(
                "BuddyDictationManager requesting initial permissions",
                category: .transcription
            )
            NSApplication.shared.activate(ignoringOtherApps: true)
        ...
        guard !Task.isCancelled else {
            LogGuru.debug(
                "BuddyDictationManager start cancelled during permission check",
                category: .transcription
            )
            isPreparingToRecord = false
            return
        ...
        guard pendingStartRequestIdentifier == startRequestIdentifier else {
            LogGuru.debug(
                "BuddyDictationManager start request was superseded",
                category: .transcription
            )
            isPreparingToRecord = false
            return
        ...
        guard !Task.isCancelled else {
            LogGuru.debug(
                "BuddyDictationManager start cancelled before recording began",
                category: .transcription
            )
            resetSessionState()
            return
        ...
        do {
            ...
            guard !Task.isCancelled else {
                LogGuru.debug(
                    "BuddyDictationManager start cancelled during session startup",
                    category: .transcription
                )
                audioEngine.stop()
                audioEngine.inputNode.removeTap(onBus: 0)
            ...
            isPreparingToRecord = false
            LogGuru.notice(
                "BuddyDictationManager recognition session started",
                category: .transcription
            )
        } catch {
            isPreparingToRecord = false
            ...
            LogGuru.error(
                "BuddyDictationManager failed to start recognition session (\(transcriptionProvider.displayName)): \(error.localizedDescription)",
                category: .transcription
            )
            resetSessionState()
        }
    ...
    private func stopPushToTalk(expectedStartSource: BuddyDictationStartSource) {
        ...
        LogGuru.debug(
            "BuddyDictationManager stop requested (\(expectedStartSource))",
            category: .transcription
        )
    ...
    private func startRecognitionSession() async throws {
        ...
        LogGuru.info(
            "BuddyDictationManager opening transcription provider \(transcriptionProvider.displayName)",
            category: .transcription
        )
        ...
        LogGuru.debug(
            "BuddyDictationManager provider ready, starting audio engine",
            category: .transcription
        )
    ...
    private func handleRecognitionError(_ error: Error) {
        ...
        if isFinalizingTranscript && !latestRecognizedText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            LogGuru.error(
                "Buddy dictation error (\(transcriptionProvider.displayName)): \(error.localizedDescription)",
                category: .transcription
            )
            lastErrorMessage = userFacingErrorMessage(
                from: error,
