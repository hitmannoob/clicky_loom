...
final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    private static var tokenProxyURL: URL? {
        URL(string: "\(CompanionManager.workerBaseURL)/transcribe-token")
    }
    ...
    func startStreamingSession(
        ...
        _ = temporaryToken
        LogGuru.notice(
            "AssemblyAI temporary token fetched for streaming session",
    ...
    private func fetchTemporaryToken() async throws -> String {
        guard let tokenProxyURL = Self.tokenProxyURL else {
            throw AssemblyAIStreamingTranscriptionProviderError(
                message: "AssemblyAI token proxy URL is not configured. Add CLICKY_WORKER_BASE_URL to .env."
            )
        }

        LogGuru.info(
            "AssemblyAI fetching temporary token from \(tokenProxyURL.absoluteString)",
            category: .transcription,
            privacy: .private
        )
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            LogGuru.error(
                "AssemblyAI token fetch failed — HTTP \(statusCode)",
                category: .transcription
            )
            LogGuru.error(
                "AssemblyAI token error body: \(body)",
                category: .transcription,
                privacy: .private
            )
            throw AssemblyAIStreamingTranscriptionProviderError(
        ...
        LogGuru.notice(
            "AssemblyAI token fetch HTTP \(httpResponse.statusCode)",
            category: .transcription
        )

        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let token = json["token"] as? String else {
            let bodyPreview = String(data: data, encoding: .utf8)?.prefix(300) ?? "unknown"
            LogGuru.error(
                "AssemblyAI token JSON did not parse. Body preview: \(bodyPreview)",
                category: .transcription,
                privacy: .private
            )
            throw AssemblyAIStreamingTranscriptionProviderError(
    ...
    private func failSession(with error: Error) {
        ...
        stateQueue.async {
            ...
            if self.isAwaitingExplicitFinalTranscript
                ...
                LogGuru.warning(
                    "AssemblyAI websocket error during active session; delivering partial transcript fallback: \(error.localizedDescription)",
                    category: .transcription
                )
            ...
            LogGuru.error(
                "AssemblyAI session failed with error: \(error.localizedDescription)",
                category: .transcription
            )
